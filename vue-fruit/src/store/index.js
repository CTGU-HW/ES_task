...
let store = new Vuex.Store({
  ...
  //   "id": 1000270,
  //   "name": "山东红富士",
  //   "name_title": "香甜可口，丰盈饱满，色泽鲜亮",
  //   "price": 99,
  //   "ali_image": "https://i.loli.net/2020/08/02/HazEolJMrg8jAf3.jpg",
  //   "limit_num": 5,//最大数目，不写可以不受限制加购物车
  //   "count":2,
  //   "checked":" "
  // }
    ],
  ...
  mutations: {
  	addCarPanelData (state,data) {
  	  let bOff = true
  	  state.carPanelData.forEach((goods) => {
  	    if(goods.pid === data[0].pid){
  	      goods.count += data[1]
  	      if(goods.count>goods.limitnum){
  	        goods.count -= data[1]
  	        state.maxOff = true
  	        bOff = false
  	        return
          ...
          state.ball.img = data[0].aliimage
  	      bOff = false
  	      state.carShow = true
  	    }
  	  })
    ...
    plusCarPanelData (state,id) {
      state.carPanelData.forEach((goods,index) => {
        if(goods.pid === id){
          if(goods.count === goods.limitum) return
          goods.count ++
          return
        }
      })
