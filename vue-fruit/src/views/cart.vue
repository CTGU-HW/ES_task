      ...
      <div class="cart-box">
        ...
        <div class="cart-inner">
          <div v-if="count<=0" class="empty-label">
            <h3>您的购物车中还没有商品</h3>
            <router-link to="/homepage" class="link">现在选购</router-link>
          </div>
          <div v-else>
                ...
                <div class="cart-top-items" v-for="(item,index) in carPanelData">
                  <div class="cart-items">
                    <div class="items-choose">
                      <span class="blue-checkbox-new" :class="{' checkbox-on':item.checked}" @click="checkGoodsHandle(item.pid)"><a></a></span>
                    </div>
                    <div class="items-thumb">
                      <img :src="item.aliimage+'?x-oss-process=image/resize,w_80/quality,Q_100/format,webp'">
                    ...
                    <div class="operation">
                      <a class="items-delete-btn" @click="delCarPanelHandle(item.pid)"></a>
                    </div>
                    <div class="subtotal">¥ {{item.price*item.count}}.00</div>
                    <div class="item-cols-num">
                      <div class="select js-select-quantity">
                        <span class="down" :class="{' down-disabled':item.count<=1}" @click="subCarPanelHandle(item.pid)">-</span>
                        <span class="num">{{item.count}}</span>
                        <span class="up" :class="{' up-disabled':item.count>=item.limitum}" @click="plusCarPanelHandle(item.pid)">+</span>
  ...
  export default {
    ...
    methods: {
      delCarPanelHandle (id) {
        ...
        this.$axios({
          method: 'delete',
          url:'http://localhost:8089/cart/deleteByPid/' + id
        }).then(function (response) {
          if (response.data === 'success'){
            _this.$alert('该商品已从购物车清空','消息',{
      ...
      delCheckGoodsHandle () {
        ...
        for (var i=0;i<items.length;i++)
        {
          if(items[i].checked==true)
          {
            ...
            this.$axios({
              method: 'delete',
              url:'http://localhost:8089/cart/deleteByPid/' + items[i].pid
            }).then(function (response) {
              this.$alert('已从购物车中删除该商品','消息',{
                confirmButtonText:'确定'
              })
            })
          }
    ...
    created() {
      let id = sessionStorage.getItem('nowUid');
      const _this = this;
      this.$axios({
        method: 'get',
        url:'http://localhost:8089/cart/selectByUid/'+id
      }).then(function (response) {
        _this.$store.state.carPanelData = response.data;
        // alert(_this.$store.state.nowUid)  //查看登陆界面是否存入值
      })
    }
  }
</script>
