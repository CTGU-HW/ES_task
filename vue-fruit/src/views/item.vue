      ...
      <div class="item-box">
        <div class="gallery-wrapper">
          <div class="gallery">
            <div class="thumb">
              <ul>
                <li class="on"><img :src="itemsInfo.aliimage"></li>
              </ul>
            </div>
          </div>
        </div>
        <div class="banner">
          <div class="sku-custom-title">
            <div class="params-price">
              <span><em>单价：&nbsp;¥</em><i>{{itemsInfo.price}}</i></span>
            </div>
            <div class="params-info">
              <h4>{{itemsInfo.pname}}</h4>
              <h6>{{itemsInfo.pnametitle}}</h6>
            </div>
          </div>
          <div class="sku-dynamic-params-panel">
            <div class="sku-dynamic-params clear">
              <div class="params-name">购买斤数</div>
              <div class="params-detail clear">
                <div class="item-num js-select-quantity">
                  <span class="down" :class="{'down-disabled':count<=1}" @click="subCount">-</span>
                  <span class="num">{{count}}</span>
                  <span class="up" :class="{'up-disabled':count>=itemsInfo.limitnum}" @click="addCount">+</span>
                </div>
              </div>
            </div>
          </div>
          <div class="sku-status">
            <div class="cart-operation-wrapper clearfix">
              <span class="blue-title-btn js-add-cart" @click="addCarPanelHandle(itemsInfo)"><a>加入购物车</a></span>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      //     return item.id == this.$route.query.itemId
      //   })[0]
      //   return itemsInfo
      // },
      maxCount () {
    ...
    methods: {
      addCount () {
        this.count++
        if(this.count>this.itemsInfo.limitnum){
          this.count = this.itemsInfo.limitnum
      ...
      addCarPanelHandle (item) {
        ...
        testCart.uid = sessionStorage.getItem('nowUid');
        if (sessionStorage.getItem('nowUid') === null){
          _this.$alert('请登录后购买','消息',{
        ...
        testCart.pid = item.pid;testCart.pname = item.pname;testCart.count = this.count;
        testCart.aliimage = item.aliimage;testCart.price = item.price;testCart.limitum = item.limitnum;
        this.$axios({
          method: 'post',
          url:'http://localhost:8089/cart/insert',
          data:testCart
    ...
    created() {
      ...
      this.$axios
        .get('http://localhost:8089/products/findById/' + id)
        .then(resp => {
          console.log(resp);
          this.itemsInfo = resp.data;
