<template>
  <div class="item">
    <div>
      <div class="item-img"><img :alt="item.pname" :src="item.aliimage" style="opacity: 1;">
      </div>
      <h6>{{item.pname}}</h6>
      <h3>{{item.pnametitle}}</h3>
      <div class="item-btns clearfix">
        <span class="item-gray-btn"><router-link :to="{name: 'Item', query: {itemId:item.pid}}">查看详情</router-link> </span>
      ...
      <div class="item-cover">
        <router-link :to="{name: 'Item', query: {itemId:item.pid}}"></router-link>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    addCarPanelHandle (item) {
      ...
      testCart.pid = item.pid;testCart.pname = item.pname;testCart.count = 1;
